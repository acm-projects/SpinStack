...
export default function MomentPickView({ moment, scrollFunc }: { moment: Moment, scrollFunc: (page: number) => void }) {
  ...
  const updateStartPosition = (value: number) => {
    if (waveWidth > 0) {
      ...
      const currentDuration = (mEnd - newStart) * moment.length;
      
      // Ensure we don't exceed max duration or overlap with mEnd
      if (currentDuration > MAX_DURATION_SECONDS) {
        const maxStart = mEnd - (MAX_DURATION_SECONDS / moment.length);
        setStart(Math.max(0, maxStart));
      } else if (newStart >= mEnd) {
  ...
  const updateEndPosition = (value: number) => {
    if (waveWidth > 0) {
      ...
      const currentDuration = (newEnd - mStart) * moment.length;
      
      // Ensure we don't exceed max duration or overlap with start
      if (currentDuration > MAX_DURATION_SECONDS) {
        const maxEnd = mStart + (MAX_DURATION_SECONDS / moment.length);
        setEnd(Math.min(1, maxEnd));
      } else if (newEnd <= mStart) {
  ...
  const currentDuration = (mEnd - mStart) * moment.length;
  const isValidDuration = currentDuration <= MAX_DURATION_SECONDS;
    ...
    <View style={{ width, justifyContent: 'center', alignItems: 'center' }}>
      ...
      >
        ...
        <View style={{ alignItems: 'flex-start', width: '100%', paddingLeft: 20 }}>
          <TouchableOpacity onPress={() => scrollFunc(-1)}>
        ...
        {/* Title */}
        <View style={{ marginTop: -45, width: '80%',}}>
          <Text style={{ fontSize: 30, fontFamily: 'Luxurious Roman', fontWeight: 'bold', textAlign: 'right', color: '#333C42' }}>
            Pick Your Moment
          </Text>
        </View>
        ...
        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
          <View style={{ justifyContent: 'center', alignItems: 'center' }}>
            <View style={{ marginLeft: 10, marginTop: -20, marginBottom: 30 }}>
              <Text style={{ fontSize: 10, fontFamily: 'Jacques Francois', color: '#333C42' }}>
              ...
              <View style={{ width: 350, justifyContent: 'center', alignItems: 'center', marginTop: 30 }}>
                ...
                <View style={{ width: '100%', marginTop: 30 }} onLayout={onWaveLayout}>
                  <Waveform
                    ...
                    baseColor="#333C42"
                    anim={false}
                    selectedColor={isValidDuration ? '#B7FFF7' : 'red'}
                  />
                  <View
                    style={{
                      position: 'absolute',
                ...
                <View style={{ marginTop: 15 }}>
                  <Text
                    style={{
                      ...
                      color: isValidDuration ? '#333C42' : 'red',
                      fontWeight: 'bold',
                    }}
                  >
                    Duration: {currentDuration.toFixed(1)}s {!isValidDuration && '(Max 30s)'}
                  </Text>
                </View>
              </View>
            </View>
        ...
        <TouchableOpacity
          style={{
            backgroundColor: isValidDuration ? '#39868F' : '#999',
            borderRadius: 10,
            borderWidth: 4,
            borderColor: '#333C42',
            alignItems: 'center',
